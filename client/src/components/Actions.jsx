...
const Actions = ({ post }) => {
  ...
  const handleLikeAndDislike = async () => {
    ...
    try {
      ...
      if (!liked) {
        // add the current user id to the post.lkes array
        ...
        //remove the current user id to the post.lkes array
        //setPost({ ...post, likes: post.likes.filter((id) => id !== user._id) });
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
            return { ...p, likes: post.likes.filter((id) => id !== user._id) };
          }
          return p;
        });
        setPosts(updatedPosts);
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setPosts(updatedPosts); // Update the state to reflect the new reply
      toast.success("You replied to this post successfully");
      onClose();
      setReplies("");
    } catch (error) {
      console.log(error);
